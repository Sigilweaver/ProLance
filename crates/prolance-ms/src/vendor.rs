...
//! converted directly into ProLance [`Spectrum`] / [`Run`] rows. This saves
...
/// is now the canonical in-memory shape every ProLance ingester (mzML reader
/// included) produces.
...
fn record_to_spectrum(run_id: &str, rec: &SpectrumRecord) -> Spectrum {
    ...
    // `ce_is_nce` flips the unit; ProLance's column is just an f32, so we drop
